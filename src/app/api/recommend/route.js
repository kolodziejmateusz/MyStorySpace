...
IMPORTANT: Return your response as a JSON array containing objects with "title" and "author" fields. Do not include any additional text, explanations, or markdown. Only return the JSON array.
...
export async function POST(req) {
  try {
    ...
    try {
      ...
      // If parsing fails, try to extract book information using regex
    ...
    const bookDetailsPromises = recommendedBooks.map(async (book) => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/books?q=${encodeURIComponent(query)}`);
